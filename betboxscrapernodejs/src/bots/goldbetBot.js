...
async function getGoldbetBalance() {
  ...
  try {
    ...
    await page.goto('https://www.goldbet.it/', {waitUntil: 'networkidle'});
    ...
    if (!isUserLoggedIn) {
      ...
      await simulateTyping(page, 'input#login_username', config.goldbet.username);

      console.log('Inserimento password');
      await simulateTyping(page, 'input#login_password', config.goldbet.password);
